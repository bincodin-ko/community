    ...
    <div className="space-y-10">
      <header>
        ...
        <p className="mt-2 text-sm text-mute">
          자동 판정({jevEnabled() ? "Jev 연결됨" : "규칙만 동작 중, Jev 키 없음"})과 신고로 올라온 글이에요. 24시간 안에 처리하는 것이 약속입니다.
        </p>
      </header>
          ...
          {posts.map((p) => (
            <li key={p.id} className="rounded-xl border border-line bg-paper p-4">
              <p className="text-sm text-mute">
                ...
                {typeof p.modBlock === "number" ? `(${Math.round(p.modBlock * 100)}%)` : ""} · 신고 {p._count.reports} · {timeAgo(p.createdAt)}
              </p>
              ...
              <form action={reviewPostAction} className="mt-3 flex flex-wrap gap-2">
                ...
                <button name="verdict" value="ok" className="btn btn-quiet">문제 없음</button>
                {p.hidden ? (
                  <button name="verdict" value="restore" className="btn btn-quiet">복구</button>
                ) : (
                  <button name="verdict" value="hide" className="btn btn-quiet hover:!border-rose hover:!text-rose">숨기기</button>
                )}
                <Link href={`/posts/${p.id}`} className="btn btn-quiet">열기</Link>
              </form>
            </li>
          ))}
        </ul>
          ...
          {comments.map((c) => (
            <li key={c.id} className="rounded-xl border border-line bg-paper p-4">
              ...
              <form action={reviewCommentAction} className="mt-3 flex flex-wrap gap-2">
                ...
                <Link href={`/posts/${c.postId}`} className="btn btn-quiet">글 열기</Link>
              </form>
            </li>
          ))}
        </ul>
      ...
      <section className="space-y-2">
        ...
        <ul className="text-sm text-mute">
          {reports.map((r) => (
            <li key={r.id}>
              {timeAgo(r.createdAt)} · {r.reason} · {r.postId ? <Link className="underline" href={`/posts/${r.postId}`}>글</Link> : "댓글"} {r.detail ? `· ${r.detail}` : ""}
            </li>
          ))}
        </ul>
      </section>
